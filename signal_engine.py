...
def telegram_signal(sig):
    ...
    if vela_time != "N/A":
        try:
            vt = datetime.fromisoformat(vela_time.replace("Z", "+00:00"))
            vt_rd = vt - timedelta(hours=4)
            diff  = (datetime.now(timezone.utc) - vt).total_seconds()
            latencia = f"Vela: {vt_rd.strftime('%H:%M:%S')} | Pub: {publish_time} | Delay: {int(diff)}s\n"
        except:
